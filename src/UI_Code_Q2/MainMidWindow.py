...
#        mydebug(f"WinMid.__init__()")    # f-string of Python 3.6+
        ...
        #Create a button to start the timer in the top right of the screen with a black background so it cannot be seen. This button can also be activated by pressing the "t" button on your keyboard.
        ...
        #Speed
        self.spinSpeed = StringVar()
        self.spinBoxSpeed = Spinbox(self.MainMidWindow, from_=0, to=180, width = 5, bg = 'snow')
        self.spinBoxSpeed.place(relx=0.05,y=250)
        self.sensorButtonSpeed = Button(self.MainMidWindow, text = 'Speed', command = self.Update_speed, bg = 'snow', height = 1)
        self.sensorButtonSpeed.place(relx=0.12, y=250)
...
#        
        #Temperature
        self.spinTemp = StringVar()
        self.spinBoxTemp = Spinbox(self.MainMidWindow, from_=0, to=180, width = 5, bg = 'snow')
        self.spinBoxTemp.place(relx=0.05,y=350)
        
        self.sensorButtonTemp = Button(self.MainMidWindow, text = 'Temp', command = self.Update_temp, bg = 'snow', height = 1)
        self.sensorButtonTemp.place(relx=0.12, y=350)
        
        #Battery
        self.spinBattery = StringVar()
        self.spinBoxBattery = Spinbox(self.MainMidWindow, from_=0, to=180, width = 5, bg = 'snow')
        self.spinBoxBattery.place(relx=0.05,y=300)
        
        self.sensorButtonBattery = Button(self.MainMidWindow, text = 'Battery', command = self.Update_battery, bg = 'snow', height = 1)
        self.sensorButtonBattery.place(relx=0.12, y=300)
...
#        self.rect.append(self.MainMidWindow.create_rectangle(WindowX/(840/720), WindowY/(840/(275*1.83)), WindowX/(840/780), WindowY/(840/((275-(spinGas))*1.83)), fill='green2'))
        ...
        self.pol = self.MainMidWindow.create_polygon(
            [      [ WindowX/2 + self.L_E  * self.size * math.cos(math.radians(spinSpeed*2.083 - 35))  *0 ,  WindowY/4 + self.L_E * self.size * math.sin(math.radians(spinSpeed*2.083 - 35)) *0 + WindowY/(840/200) ], #bottom put to 0 to make it an arrow
                   [ WindowX/2 + self.L_S  * self.size * math.cos(math.radians(spinSpeed*2.083 + 55)) , WindowY/4 + self.L_S * self.size*  math.sin(math.radians(spinSpeed*2.083 + 55)) + WindowY/(840/200) ],
                   [ WindowX/2 + self.L_W  * self.size * math.cos(math.radians(spinSpeed*2.083 + 145)), WindowY/4 + self.L_W * self.size * math.sin(math.radians(spinSpeed*2.083 + 145)) + WindowY/(840/200) ] ,  
                   [ WindowX/2 + self.L_N *  self.size * math.cos(math.radians(spinSpeed*2.083 + 235)), WindowY/4 + self.L_N  * self.size * math.sin(math.radians(spinSpeed*2.083 + 235)) + WindowY/(840/200) ]
            ] , fill='red'        ) 
...
#        self.line = self.MainMidWindow.create_line( WindowX/(840/(330+self.angle)), WindowY/(840/50) , WindowX/(840/(330+self.angle)), WindowY/(840/90), fill = "red" )
        ...
        if (float(spinTemp) < 90): 
#            self.temperature.set(str(self.spinBox.get()))
            self.temperature.set(str(spinTemp) + " C" + degree_sign)
            self.tempLabel.config(font=("Courier 30 bold"),bg='#%02x%02x%02x' % (75+int(round(spinTemp*2)), 75+int(round(spinTemp*2)), 255))
        ...
        #Temperature value has to be between 0 and 255 and a whole number otherwise the color of the label won't work
            ...
            self.tempLabel.config(font=("Courier 30 bold"),bg='#%02x%02x%02x' % (255, 255+180-int(round(spinTemp)*2), 255+180-int(round(spinTemp)*2)))
            self.tempLabel.place(relx=0.18, rely=0.7)
            self.temperature.set("Warning "+ str(spinTemp) + " C" + degree_sign)
