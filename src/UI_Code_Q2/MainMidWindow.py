...
#        mydebug(f"WinMid.__init__()")    # f-string of Python 3.6+
        ...
        self.spinBox = Spinbox(self.MainMidWindow, from_=0, to=180, width = 5, bg = 'snow')
        self.spinBox.place(x=150,y=150)
        
        self.spinTemp = StringVar()
        self.spinBox = Spinbox(self.MainMidWindow, from_=0, to=180, width = 5, bg = 'snow')
        self.spinBox.place(x=150,y=100)
        
        self.sensorButton = Button(self.MainMidWindow, text = 'Load', command = self.useSensorData, bg = 'snow', height = 1)
        self.sensorButton.place(x=220, y=200)
        
        self.sensorButton = Button(self.MainMidWindow, text = 'Load', command = self.useSensorData, bg = 'snow', height = 1)
        self.sensorButton.place(x=220, y=150)
        
        self.sensorButtonTemp = Button(self.MainMidWindow, text = 'Load', command = self.useSensorData, bg = 'snow', height = 1)
        self.sensorButtonTemp.place(x=220, y=100)
...
#       self.my_rectangle = self.round_rectangle(40, 250-((220-self.angle-20)/220)*200, 140, 250 , radius=20, fill="red3")
        ...
        self.line = self.MainMidWindow.create_line( 770 * math.cos(math.radians(self.angle)), 50 , 770 * math.cos(math.radians(self.angle)), 100 , 771 * math.cos(math.radians(self.angle)), 50 , 771 * math.cos(math.radians(self.angle)), 100, fill = "red" )
...
#            self.temperature.set(" Warning "+str(self.angle+20)+" C"+degree_sign)
    ...
    def useSensorData(self):
        self.spinTemp = float(self.spinBox.get()) #Get the value of the spinbox and save it as a float
